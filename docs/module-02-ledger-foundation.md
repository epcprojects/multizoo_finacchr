...
| `Account` | `code` (unique), `name`, `type` (`ASSET`/`LIABILITY`/`EQUITY`/`INCOME`/`EXPENSE`), `subtype` (`CASH`, `BANK`, `WALLET`, `RESERVE`, `RECEIVABLE`, `PAYABLE`, `EQUITY`, `INCOME`, `EXPENSE`), `businessUnitId` (nullable = group-wide), `parentId` (nullable), `isPostable`, `isSystem` | `BaseEntity`. `isSystem` accounts (e.g. Opening Balance Equity) can't be renamed/deactivated |
...
apps/api/src/app/modules/
  ...
  accounts/         entity, chart-of-accounts.ts (seed template + provisioning), CRUD, auto-coding
